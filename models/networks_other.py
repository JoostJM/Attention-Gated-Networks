...
def get_scheduler(optimizer, last_epoch=-1, **sched_opts):
  logger = logging.getLogger('scheduler')
  lr_policy = sched_opts['name']
  scheduler_opts = sched_opts['parameters']
  logger.info('lr_policy = [%s]', lr_policy)
  if last_epoch == 0:
    last_epoch = -1

  if lr_policy == 'lambda':
    def lambda_rule(epoch):
      lr_l = 1.0 - max(0, epoch + 1 + sched_opts['lambda']['n_epoch'] - sched_opts['lambda']['n_iter']) / \
             float(sched_opts['lambda']['n_iter_decay'] + 1)
      return lr_l

    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=last_epoch)
  elif lr_policy == 'step':
    scheduler = lr_scheduler.StepLR(optimizer, last_epoch=last_epoch, **scheduler_opts)
  elif lr_policy == 'plateau':
    print('schedular=plateau')
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, **scheduler_opts)
