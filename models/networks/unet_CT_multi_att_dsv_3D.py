...
class unet_CT_multi_att_dsv_3D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if 1 in self.block_gpu_ids:  # Move from 1 to 2
            maxpool1.to(self.block_gpu_ids[1][1])

        conv2 = self.conv2(maxpool1)
        maxpool2 = self.maxpool2(conv2)

        if 2 in self.block_gpu_ids:  # Move from 2 to 3
            maxpool2.to(self.block_gpu_ids[2][1])

        conv3 = self.conv3(maxpool2)
        maxpool3 = self.maxpool3(conv3)

        if 3 in self.block_gpu_ids:  # Move from 3 to 4
            maxpool3 = maxpool3.to(self.block_gpu_ids[3][1])
        ...
        if 4 in self.block_gpu_ids:  # Move to block 5 (final block)
            dsv4 = dsv4.to(self.block_gpu_ids[4])
            dsv3 = dsv3.to(self.block_gpu_ids[4])
            dsv2 = dsv2.to(self.block_gpu_ids[4])
            dsv1 = dsv1.to(self.block_gpu_ids[4])
    ...
    def split_net(self, devices):
        ...
        # Block #1
        self.conv1.to(block_devices[1])
        self.maxpool1.to(block_devices[1])

        # Block #2
        self.conv2.to(block_devices[2])
        self.maxpool2.to(block_devices[2])

        # Block #3
        self.conv3.to(block_devices[3])
        self.maxpool3.to(block_devices[3])

        # Block #4
        self.conv4.to(block_devices[4])
        self.maxpool4.to(block_devices[4])
        self.center.to(block_devices[4])
        self.gating.to(block_devices[4])

        self.attentionblock4.to(block_devices[4])
        self.up_concat4.to(block_devices[4])

        # Block #3
        self.attentionblock3.to(block_devices[3])
        self.up_concat3.to(block_devices[3])
        self.dsv4.to(block_devices[3])

        # Block #2
        self.attentionblock2.to(block_devices[2])
        self.up_concat2.to(block_devices[2])
        self.dsv3.to(block_devices[2])

        # Block #1
        self.up_concat1.to(block_devices[1])
        self.dsv2.to(block_devices[1])
        self.dsv1.to(block_devices[1])

        # Block #5
        self.final.to(block_devices[5])
