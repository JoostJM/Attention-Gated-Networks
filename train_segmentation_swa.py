...
def train(arguments):
    ...
    try:
        ...
        if model.which_epoch < init_n_epoch:
            logger.info('Pretraining the model for %i epochs', init_n_epoch - model.which_epoch + 1)
            for epoch in range(model.which_epoch, init_n_epoch + 1):
        ...
        if train_opts.swa:
            ...
            for epoch in range(epoch + 1, train_opts.n_epochs):
              logger.info('(epoch: %d, total # iters: %d)' % (epoch, len(train_loader)))
